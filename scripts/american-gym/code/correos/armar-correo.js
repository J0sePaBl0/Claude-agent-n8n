...
// contradecirse.
// Colores de la marca en el encabezado, el bloque de la cita y el pie.
// PENDIENTE (info de American Gym): los colores reales. Hoy son los de Dulce María.
const MARCA = { primario: '#0E6B5A', suave: '#E9F5F4', borde: '#A2D4CA' };
// Política de cancelación. Tiene que coincidir con "Preparar gestión" del motor.
// `CARGO = null` = sin cargo, y el correo no menciona montos.
const CARGO = null;
...
// Todo lo que venga de la hoja o del cliente termina dentro del HTML del correo.
const esc = (v) => txt(v)
  .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  .replace(/"/g, '&quot;');

const tipo = txt(e.tipo);
const nombre = esc(e.nombre_cliente) || 'Cliente';
const servicio = esc(e.servicio);
const cita = esc(e.texto_cita);
const entrenador = esc(e.entrenador);
const anterior = esc(e.texto_anterior);
const nota = esc(e.nota);
const negocio = esc(e.sede_nombre) || 'American Gym';
...
const PLANTILLAS = {
  nueva: {
    ...
    entrada: `Recibimos su solicitud de cita para <strong>${servicio}</strong>. `
      + 'Aún no está confirmada: el gimnasio se la confirma por WhatsApp 24 horas antes.',
  ...
  reprogramada: {
    ...
    entrada: `Su cita de <strong>${servicio}</strong> quedó reprogramada. `
      + 'El gimnasio se la confirma por WhatsApp 24 horas antes de la nueva fecha.',
    color: '#8A5A00',
...
const bloqueCita = `
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0"
         style="background:${MARCA.suave};border:1px solid ${MARCA.borde};border-radius:8px;padding:18px 20px;">
    <tr><td>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
        ${filaDato('Servicio', servicio)}
        ${filaDato(tipo === 'cancelada' ? 'Era el' : 'Fecha y hora', cita)}
        ${tipo === 'reprogramada' ? filaDato('Antes era', anterior) : ''}
        ${filaDato('Entrenador', entrenador)}
...
const cierre = tipo === 'cancelada'
  ...
  : 'Si necesita confirmarla, moverla o cancelarla, escríbanos por WhatsApp.'
    + (CARGO !== null
      ? ` Recuerde que los cambios con menos de ${HORAS_AVISO} horas de aviso generan un cargo de `
        + `${CARGO.toLocaleString('es-CR')} colones.`
      : '');
...
const bloqueSede = (direccion || telefono)
  ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0"
       ...
       <tr><td style="font-size:13px;color:#556963;line-height:1.6;">
         ${direccion ? `<strong style="color:#17322C;">${negocio}</strong><br>${direccion}<br>` : ''}
         ${maps ? `<a href="${esc(maps)}" style="color:${MARCA.primario};">Ver en el mapa</a><br>` : ''}
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0"
        ...
        <tr><td style="background:${p.color};padding:22px 28px;">
          <div style="color:#ffffff;font-size:18px;font-weight:700;letter-spacing:.2px;">${negocio}</div>
        </td></tr>
        ...
        <tr><td style="background:${MARCA.suave};border-top:1px solid ${MARCA.borde};padding:16px 28px;
